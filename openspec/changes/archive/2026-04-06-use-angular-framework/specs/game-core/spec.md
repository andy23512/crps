...
The system SHALL recognize exactly three valid moves: `rock`, `paper`, and `scissors`. Any other value SHALL be treated as invalid. Angular components SHALL use this validation through a dedicated game logic service before round resolution.
...
The system SHALL determine the winner of a round using the standard rules: rock beats scissors, scissors beats paper, paper beats rock. Winner determination SHALL be executed by Angular service logic, not hardcoded in templates.
