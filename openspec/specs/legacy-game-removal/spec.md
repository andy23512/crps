...
The repository SHALL remove the legacy static implementation under `game/` and SHALL keep the Angular implementation at repository root as the only supported game runtime.
...
- **AND** Angular workspace files at repository root SHALL remain present
...
- **THEN** all prescribed commands SHALL target the repository root workspace
